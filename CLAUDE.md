...
- **The export carries it the same way `field.styles` does**: `styles: row.styles` in `mapRows` (`exportForm.utils.ts`), so a row with no styles serializes with no `styles` key at all (`JSON.stringify` drops `undefined`), and `ExportedRow.styles` is optional for the same reason `ExportedField.styles` is not — a field's styles object always exists (`{}` at minimum), a row's does not.
- **Inherits both open simulator gaps that `field.styles` already has, undiscussed and unfixed for rows too**: the simulator does not apply row styles any more than it applies field styles (`grep .styles under components/organisms/preview/` still returns nothing), and `customClasses` on a row hits the identical Tailwind-v4-scans-source-files wall as `field.styles.customClasses` and `tooltip.customClasses` — a class typed in the row's popover works only if some other component already happens to reference it. Same fix, same cost, not paid here.
- **Margins apply as inline `style`, `customClasses` appends to the `<li>`'s className** — exactly how `CanvasFieldChip` applies `field.styles`, for the same reason: Tailwind can't be handed a margin value from a variable, but it can be handed a whole class list.
- **`RowDragPreview` does not reflect `row.styles`.** The ghost overlay while dragging a row is deliberately generic (fixed orange border, no per-row look — see "Reordering rows") to signal "this is what's flying," not a faithful preview of the row's own appearance. Applying custom classes there would fight that signal.
