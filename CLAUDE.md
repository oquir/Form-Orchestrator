...
- **Bulk delete asks for confirmation** (`confirmFieldRemoval`, 2+ fields, shared by Supr and the bar's button) because there is no undo; single deletes (chip ✕, row ✕, context menu) still don't.
