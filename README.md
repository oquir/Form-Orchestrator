...
El tooltip lleva `content`, `position` (`top` | `bottom` | `left` | `right`) y un `customCss` opcional (CSS plano, no clases de Tailwind — ver más abajo). El `content` es **el mismo `RichTextContent` estructurado que usa `rich_text`**, así que admite negrita, cursiva, subrayado y enlaces, pasa por el mismo sanitizador y el consumidor lo pinta con los mismos componentes — nunca con `dangerouslySetInnerHTML`.
